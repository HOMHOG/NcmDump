...
const char NeteaseCrypt::mPng[8] = {static_cast<char>(0x89), 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A};
...
void NeteaseCrypt::Dump() {
	...
	// 	replace(mDumpFilepath, "|", "｜");
	// } else {
	mDumpFilepath = mDumpFilepath + fileNameWithoutExt(mFilepath);
	// }
...
NeteaseCrypt::NeteaseCrypt(std::string const& path, std::string const& dumpPath) {
	if (!openFile(path)) {
